...
use marlu::{
    ...
    ndarray::{Array3},
    Jones, 
    io::{ms::{MeasurementSetWriter}, VisWritable}, RADec, Complex
};
...
fn bench_ms_init_mwax_half_1247842824(crt: &mut Criterion) {
    ...
    let mwalib_timestep_range = first_common_timestep_idx..(first_common_timestep_idx + NUM_TIMESTEPS);
    ...
    crt.bench_function("ms_writer::initialize_from_mwalib - mwax_half_1247842824", |bch| {
        bch.iter(|| {
            ms_writer
...
fn synthesize_test_data(shape: (usize, usize, usize)) -> (Array3<Jones<f32>>, Array4<f32>, Array4<bool>) {
    let jones_array = Array3::from_shape_fn(
        shape,
    ...
    let weight_array = Array4::from_shape_fn(
        ...
        |(timestep_idx, chan_idx, baseline_idx, pol_idx)| {
            (timestep_idx * shape_with_pol.0 * shape_with_pol.1 * shape_with_pol.2 * shape_with_pol.3
...
fn bench_ms_write_mwax_part_1247842824(crt: &mut Criterion) {
    ...
    let mwalib_baseline_idxs: Vec<usize> = (0..get_baseline_count(context.metafits_context.num_ants)).collect();
    ...
    crt.bench_function("ms_writer::write_vis_mwalib - mwax_half_1247842824", |bch| {
        bch.iter(|| {
            ms_writer
            .write_vis_mwalib(
                jones_array.view(),
